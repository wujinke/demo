...
redisModel.set=function(data){
  client3.hset(data.user_id,data.title_id,data.contents);
}
redisModel.get=function(data,callback){
  client3.hmget(data.user_id,function(error,res){
          if(error) {
            console.log(error);
          } else {
            console.log(res);
